...
const Task = () => {
  ...
  if (taskItems.length === 0) {
    return <h1 className="text-5xl text-amber-50">There is no Task</h1>;
  }
  return (
      ...
      <ul className="space-y-5">
        {taskItems.map((t) => (
          <li
            key={t.id}
            id={t.id}
            className={`w-200 py-5 px-5 text-2xl flex justify-between ${
              t.done ? "bg-amber-700" : "bg-blue-900"
            }`}
          >
            <h1>{t.title}</h1>
            <div className="flex gap-5 items-center">
              ...
              {t.done ? (
                <BsXSquareFill onClick={() => handleDoUndo(t.id)} />
              ) : (
                <BsCheckSquareFill onClick={() => handleDoUndo(t.id)} />
              )}
            </div>
          </li>
        ))}
